##############################################################################
# Code to reproduce the multistate survival model in 
# Soriano-Redondo et al. Demographic rates reveal the 
# benefits of protected areas in a long-lived migratory bird (submitted to PNAS).
 # Developed by: Dr Fitsum Abadi Gebreselassie
...
CH1 <- as.matrix(read.csv2('Whooper_MS_Age_Encounter.csv',header=F,sep=","))
...
f <- apply(CH, 1, get.first) ### Tell the model when was the time of first capture
...
# Multistate model that accounts for trap-dependence (Adapted from Kéry & Schaub 2012))
